...
function hexToRgb(hex: string): { r: number; g: number; b: number } {
  ...
  if (Number.isNaN(n)) return { r: 255, g: 248, b: 241 };
...
export function normalizeColorHex(value: string, fallback: string): string {
...
export type ColorFieldVariant = "field" | "swatch" | "fill";
...
export function ColorField({
  ...
  const color = normalizeColorHex(value || fallback, fallback);
  const initial = rgbToHsv(hexToRgb(color).r, hexToRgb(color).g, hexToRgb(color).b);
  ...
  const fillText = contrastTextColor(color);

  const panel = open ? (
    <div
      id={panelId}
      className={`absolute z-50 space-y-3 rounded-2xl border border-black/10 bg-white p-3 shadow-[0_12px_40px_rgba(0,0,0,0.12)] ${
        variant === "swatch"
    ...
    return (
      <div ref={rootRef} className="relative">
        ...
        <button
          ...
          onClick={() => setOpen((o) => !o)}
          className="relative mt-0.5 h-9 w-9 rounded-[10px] border-2 border-black transition-transform active:scale-95"
    ...
    return (
      <div ref={rootRef} className="relative w-full">
        <button
          ...
          style={{ backgroundColor: color, color: fillText }}
        >
          <span className="pointer-events-none text-sm font-extrabold uppercase tracking-wide">
            {label}
          </span>
        </button>
        {panel}
