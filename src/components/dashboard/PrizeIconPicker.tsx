...
function IconPickerPanel({
 ...
 className={`z-50 flex max-h-[min(70vh,28rem)] flex-col overflow-hidden rounded-[16px] border-2 border-black bg-white ${className ?? ""}`}
 style={style}
 >
...
export function PrizeIconPicker({
 ...
 open ? "!-translate-x-0.5 !-translate-y-0.5 " : ""
 }`}
 >
 <PrizeWheelIcon icon={selected} size={30} />
 <span className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-[8px] border-2 border-black bg-[var(--c-lavender)] text-ink transition group-hover:-translate-y-px">
