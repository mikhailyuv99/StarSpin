...
function RarityPanel({
 ...
 className="z-[60] flex max-h-[min(70vh,20rem)] flex-col overflow-hidden rounded-[16px] border-2 border-black bg-white "
 style={style}
 >
...
export function PrizeRaritySelect({
 ...
 aria-controls={panelId}
  className={`${triggerBase} !min-h-0 !justify-between !px-[0.9rem] !py-[0.55rem] text-left ${
 ...
 aria-hidden
 />
 <span className="truncate text-sm font-extrabold leading-none text-ink">
 {t(`dashboard.prizeRarity_${value}`)}
 </span>
 </span>
 <span
 className={`ml-2 flex h-5 w-5 shrink-0 items-center justify-center rounded-[7px] border-2 border-black bg-[var(--c-lavender)] text-ink transition ${
 open ? "rotate-180" : ""
 }`}
 aria-hidden
 >
